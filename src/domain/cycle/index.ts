...
const sortedStarts = (records: CycleRecord[]) =>
  records
    .map((r) => parseISO(r.periodStartDate))
...
export function calculateMedianCycleLength(
  records: CycleRecord[],
): number | null {
  ...
  const lengths = records
    .flatMap((r) =>
      r.periodEndDate
        ? [
            differenceInCalendarDays(
              parseISO(r.periodEndDate),
              parseISO(r.periodStartDate),
            ) + 1,
          ]
        : [],
    )
...
export function estimateOvulationWindow(
  start: string,
  cycleLength: number,
  lutealLength = 14,
  uncertainty = 1,
  ...
  return {
    start: subDays(center, uncertainty),
    end: addDays(center, uncertainty),
  };
}

export function estimatePhaseRanges(
  start: string,
  cycleLength: number,
  periodLength: number,
  lutealLength = 14,
  uncertainty = 1,
): Record<CyclePhase, DateRange> {
  const day1 = parseISO(start);
  const ovulation = estimateOvulationWindow(
    start,
    cycleLength,
    lutealLength,
    uncertainty,
  ...
  return {
    ...
    follicular: {
      start: addDays(day1, periodLength),
      end: subDays(ovulation.start, 1),
    },
    ovulation,
    luteal: {
      start: addDays(ovulation.end, 1),
      end: addDays(day1, cycleLength - 1),
    },
  };
}

export function estimateCurrentPhase(
  day: number,
  cycleLength: number,
  periodLength: number,
  lutealLength = 14,
...
export function estimateNextPeriodDate(
  start: string,
  cycleLength: number,
  variability = 0,
): DateRange {
...
export function detectPossiblyIrregularCycle(records: CycleRecord[]): boolean {
  return calculateCycleVariability(records) > 7;
}
...
export function calculatePredictionConfidence(
  ...
  const missingEnds =
    records.filter((r) => !r.periodEndDate).length > records.length / 2;
...
export function validateCycleDates(
  ...
  if (isAfter(start, startOfDay(today)))
    errors.push("لا يمكن أن يكون تاريخ البداية في المستقبل.");
  if (record.periodEndDate && isBefore(parseISO(record.periodEndDate), start))
    errors.push("يجب أن تكون نهاية الحيض بعد البداية.");
  if (existing.some((r) => r.periodStartDate === record.periodStartDate))
    errors.push("يوجد سجل بهذا التاريخ بالفعل.");
  return errors;
}
...
export function estimateCycle(
  ...
  const cycleLength = Math.round(
    enough
      ? (calculateMedianCycleLength(records) ?? 28)
      : (settings.expectedCycleLength ?? 28),
  );
  const periodLength = Math.round(
    calculateAveragePeriodLength(records) ?? settings.expectedPeriodLength ?? 5,
  );
  const variability = calculateCycleVariability(records);
  ...
  return {
    cycleDay,
    phase: estimateCurrentPhase(cycleDay, cycleLength, periodLength, luteal),
    confidence: calculatePredictionConfidence(records, today),
    nextPeriod: estimateNextPeriodDate(
      latest.periodStartDate,
      cycleLength,
      variability,
    ),
    ovulation: estimateOvulationWindow(
      latest.periodStartDate,
      cycleLength,
      luteal,
      clamp(Math.ceil(variability / 4), 1, 4),
    ),
    phaseRanges: estimatePhaseRanges(
      latest.periodStartDate,
      cycleLength,
      periodLength,
      luteal,
    ...
    expectedPeriodLength: periodLength,
    irregular: detectPossiblyIrregularCycle(records),
  };
}
